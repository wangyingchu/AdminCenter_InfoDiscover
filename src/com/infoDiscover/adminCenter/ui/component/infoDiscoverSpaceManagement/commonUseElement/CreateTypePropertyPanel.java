...
/**
 * Created by wangychu on 10/12/16.
 */
public class CreateTypePropertyPanel extends VerticalLayout {
    ...
    public CreateTypePropertyPanel(UserClientInfo userClientInfo){
    ...
    public void attach() {
        ...
        if(this.getPropertyTypeKind().equals(InfoDiscoverSpaceOperationUtil.TYPEKIND_DIMENSION)){
            sectionActionBarLabel=new Label(FontAwesome.CUBE.getHtml()+" "+getDiscoverSpaceName()+" /"+FontAwesome.TAGS.getHtml()+" "+this.getTypeName() , ContentMode.HTML);
        }
    ...
    public void addNewTypeProperty(){
        ...
        boolean isExistTypePropertyName= InfoDiscoverSpaceOperationUtil.
                checkTypePropertyExistence(getDiscoverSpaceName(), getPropertyTypeKind(), getTypeName(), typePropertyNameStr);
        if(isExistTypePropertyName){
            ...
            if(InfoDiscoverSpaceOperationUtil.TYPEKIND_DIMENSION.equals(getPropertyTypeKind())){
                checkErrorMessage="维度类型 "+getTypeName()+" 中已经存在属性 "+typePropertyNameStr;
            }
            if(InfoDiscoverSpaceOperationUtil.TYPEKIND_FACT.equals(getPropertyTypeKind())){
                checkErrorMessage="事实类型 "+getTypeName()+" 中已经存在属性 "+typePropertyNameStr;
            }
            if(InfoDiscoverSpaceOperationUtil.TYPEKIND_RELATION.equals(getPropertyTypeKind())){
                checkErrorMessage="关系类型 "+getTypeName()+" 中已经存在属性 "+typePropertyNameStr;
            }
        ...
        if(InfoDiscoverSpaceOperationUtil.TYPEKIND_DIMENSION.equals(getPropertyTypeKind())){
            confirmMessageString=" 请确认在维度类型 "+getTypeName()+" 中添加类型属性 "+typePropertyNameStr;
        }
        if(InfoDiscoverSpaceOperationUtil.TYPEKIND_FACT.equals(getPropertyTypeKind())){
            confirmMessageString=" 请确认在事实类型 "+getTypeName()+" 中添加类型属性 "+typePropertyNameStr;
        }
        if(InfoDiscoverSpaceOperationUtil.TYPEKIND_RELATION.equals(getPropertyTypeKind())){
            confirmMessageString=" 请确认在关系类型 "+getTypeName()+" 中添加类型属性 "+typePropertyNameStr;
        }
        ...
        final CreateTypePropertyPanel self=this;
            ...
            public void buttonClick(final Button.ClickEvent event) {
                ...
                if(createTypePropertyResult){
                    ...
                    Notification resultNotification = new Notification("添加数据操作成功",
                            "创建类型属性成功", Notification.Type.HUMANIZED_MESSAGE);
                    resultNotification.setPosition(Position.MIDDLE_CENTER);
                    resultNotification.setIcon(FontAwesome.INFO_CIRCLE);
                    resultNotification.show(Page.getCurrent());
                }else{
                    Notification errorNotification = new Notification("创建类型属性错误",
