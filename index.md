...
# Info Discover 高价值密度知识分析系统架构设计与业务组件

## ❏ 系统知识获取数据平台

Info Discover　高价值密度知识获取数据平台是一个运行在大数据环境下，对各种异构数据源中的数据进行集中分析并将分析结果使用`知识型数据存储库`知识化并持久化保存的数据分析体系结构。它是一套异构的基于TCP/IP网络传输协议的信息系统。该系统使用 ***Apache Kafka*** 分布式消息系统作为核心信息传输、交换枢纽。它提供了一套标准化的数据输入方式和封装格式,通过使用标准化的数据封装格式（基于***Apache Avro***技术），实现了与操作系统无关，编程实现无关的数据交换体系。
...
**数据采集单元**: 运行在各类信息系统硬件环境中的各种类型的为高价值密度知识获取数据平台提供输入数据的数据源称为数据采集单元。数据采集单元通过内置的标准化数据发送代理程序（使用Java与Web2.0技术开发，可以作为嵌入式代理运行在任意支持Java技术的硬件环境中）使用标准的数据输入方式和封装格式将不同类型和结构的业务数据封装成标准格式的`数据负载`并输入到数据平台的 Kafka 数据总线中。

**数据分析单元**: 数据分析单元的主要功能是从 Kafka 数据总线中获取数据负载，并将数据负载中的业务数据信息分析、计算以产生最终判断结论，并将该结论发送到知识型数据存储系统中以创建领域知识型数据记录。数据分析单元具有群组通讯功能，每一个数据分析单元原则上都运行在一个单独的计算机硬件环境中。若干个数据分析单元共同构成一个数据分析单元网络集群（基于***Apache Ignite技术***），该集群内的数据分析单元可以互相识别，当新的数据分析单元启动或旧有数据分析单元关闭时，该集群内的所有其他分析单元都会自动获取相应消息提示信息。

**知识型数据存储系统**: 知识型数据存储系统是一个使用Java技术开发的知识型数据存储与分析服务系统，它使用OrientDB图数据库作为后端数据存储引擎，实现了Info Discover 高价值密度知识分析系统的核心数据模型（基于图形数据库的星型模型），并提供了一系列的供数据存储与分析使用的API。该服务系统存储来自数据分析单元的知识型数据信息，并为外部的知识型数据使用单元提供各种接口类型的数据服务。除了具备图形数据模型特有的功能外，系统还具有与现有的数据库事实技术标准（关系数据库系统）相一致的功能,能够使用行业标准的方式（例如使用SQL语言或JDBC）进行数据操作。通过使用OrientDB提供的数据存储支持，系统还能够支持海量数据，提供快速高效的数据访问，并且能够通过透明的硬件水平扩展来提高整体的吞吐能力和性能。


## ❏ 系统数据管理与分析操作终端

Info Discover　高价值密度知识分析系统内建有通过Web浏览器访问的数据管理与知识分析的用户界面(基于***Vaadin技术***)。通过使用该操作界面，用户可以便捷的进行系统管理，知识数据操作以及业务知识分析等各种操作。


**数据管理功能：*****通过使用操作终端的数据管理功能，用户可以创建，维护，删除Info Discover高价值密度知识分析系统中的顶级信息发现空间;监控事实，维度以及关系类型的实时数据存储;管理各类数据类型的数据元数据定义以及维护供数据关联关系自动创建所用的规则定义。***
...
◼︎ 下图为空间关系类型数据分布统计界面，通过使用数据分布统计界面，用户可以实时获取维度数据，事实数据和关系数据的数据总量以及不同类型数据的数据分布情况。
![数据处理流程](pic/Screenshot2.png)


◼︎ 下图为空间事实类型定义管理界面，通过使用数据类型定义界面，用户可以根据业务需要创建各类事实，维度以及关系类型，并为各个数据类型创建数据属性。
![数据处理流程](pic/Screenshot3.png)


◼︎ 下图为数据关联规则定义管理界面，通过定义数据关联规则，系统能够在获取到新的知识数据后自动根据规则建立不同知识数据之间的数据关联。
![数据处理流程](pic/Screenshot17.png)


◼︎ 下图为创建事实数据界面，除了从数据总线自动获取知识数据外，用户也可以通过操作终端界面手动创建各类知识数据并为数据建立关联关系。
![数据处理流程](pic/Screenshot18.png)


**知识分析功能：*****通过使用操作终端的知识分析功能，用户可以按照特定数据属性值查询批量获取知识数据的详细细节，宏观分析知识数据的统计性特征。或者通过微观分析特定的知识数据，以关联信息探索的方式获取该数据与其他数据间的关系关联，查找相似数据或者若干数据之间的相互关联。***

◼︎ 下图为知识数据查询界面，通过使用查询界面，用户可以使用数据属性过滤的方式批量查询获取知识数据。
![数据处理流程](pic/Screenshot4.png)


◼︎ 下图为知识数据详细信息界面，通过使用该界面，用户可以编辑特定知识数据中包含的属性信息，管理该知识数据与其他数据之间的关联关系信息。
![数据处理流程](pic/Screenshot9.png)


◼︎ 下图为知识数据详细信息界面中的数据关联网络图面板，该面板以图形化的方式展示了知识数据与其他数据之间的关联关系。
![数据处理流程](pic/Screenshot10.png)


◼︎ 通过点击数据关联网络图中的不同数据节点，可以动态的扩展展示额外的知识数据节点间的关联关系。
![数据处理流程](pic/Screenshot11.png)


◼︎ 下图为３维空间数据分布＋数据密度分析界面，该功能能够在３维空间中以立方体加颜色密度的可视化方式实时展示选定的批量知识数据的特定标量属性值之间的关系。
![数据处理流程](pic/Screenshot5.png)


◼︎ 下图为２维平面数据气泡图分析界面，该功能能够在２维平面坐标中以气泡图的形式实时展示选定的批量知识数据的特定标量属性值之间的关系。
![数据处理流程](pic/Screenshot6.png)


◼︎ 下图为３维空间数据属性值散点分布分析界面，该功能能够在３维空间中以颜色分组的形式实时展现选定的批量知识数据的某一特定属性值的空间分组信息。
![数据处理流程](pic/Screenshot7.png)


◼︎ 下图为时间序列数据分析界面，该功能能够在时间轴上实时展示选定的批量知识数据的特定标量属性值之间的关系。
![数据处理流程](pic/Screenshot8.png)


◼︎ 下图为知识关联数据探索界面，该功能能够图形化的显示某一特定的知识数据，通过点击其他相关连的知识数据节点，用户可以以交互式的方式探索与其相关的其他知识数据以获取隐含的业务信息。
![数据处理流程](pic/Screenshot13.png)


◼︎ 下图为多项数据间属性值比较功能界面，通过使用此功能，用户可以选定若干知识数据，并以多种方式图形化的比较不同的数据属性。
![数据处理流程](pic/Screenshot14.png)


◼︎ 下图为两项数据间关联关系发现界面，通过使用该功能，用户可以选择两个不同的知识数据并获取两者之间的最短路径，最长路径以及所有路径等关联关系信息。
![数据处理流程](pic/Screenshot15.png)


◼︎ 下图为相似数据探索界面，通过使用该功能，用户可以选择某一特定的知识数据，并根据选定的关联维度查找与其类似的其他知识数据。
...
[数据管理与知识分析系统(https://github.com/wangyingchu/AdminCenter_InfoDiscover)](https://github.com/wangyingchu/AdminCenter_InfoDiscover)

[数据分析与报表服务系统(https://github.com/wangyingchu/InfoAnalyseService_InfoDiscover)](https://github.com/wangyingchu/InfoAnalyseService_InfoDiscover)

[知识型数据存储引擎(https://github.com/wangyingchu/DiscoverEngine_InfoDiscover)](https://github.com/wangyingchu/DiscoverEngine_InfoDiscover)

[数据关联关系管理规则引擎(https://github.com/wangyingchu/RuleEngine_InfoDiscover)](https://github.com/wangyingchu/RuleEngine_InfoDiscover)
