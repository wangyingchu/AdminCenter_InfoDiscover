...
**知识型数据存储系统**: 知识型数据存储系统是一个使用Java技术开发的知识型数据存储与分析服务系统，它使用OrientDB图数据库作为后端数据存储系统，实现了Info Discover 高价值密度知识分析系统的核心数据模型（基于图形数据库的星型模型），并提供了一系列的供数据存储与分析使用的API。该服务系统存储来自数据分析单元的知识型数据信息。并为外部的知识型数据使用单元提供各种接口类型的数据服务。除了具备图形数据模型特有的功能外，系统还具有与现有的数据库事实技术标准（关系数据库系统）相一致的功能，具有标准的数据访问接口，能够使用行业标准的方式（例如使用SQL语言）进行数据操作。通过使用OrientDB提供的数据存储支持，系统还能够支持海量数据，提供快速高效的数据访问，并且能够通过透明的硬件水平扩展来提高整体的吞吐能力和性能。
